 ...
 */
class SM_Log_Handler_DB extends SM_Log_Handler {
	 ...
	 * }
	 *
	 * @see SM_Log_Handler_DB::get_log_source() for default source.
	 *
	...
	protected static function add( $timestamp, $level, $message, $source, $context ) {
		...
		$insert = array(
			...
			'level'     => SM_Log_Levels::get_level_severity( $level ),
			'message'   => $message,
			'source'    => $source,
		);
		...
		return false !== $wpdb->insert( "{$wpdb->prefix}sm_log", $insert, $format );
	}
	...
	public static function flush() {
		...
		return $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}sm_log" );
	}
	...
	public static function delete( $log_ids ) {
		...
		$query = $wpdb->prepare(
			"DELETE FROM {$wpdb->prefix}sm_log WHERE log_id IN {$query_in}",
			$log_ids
		);
	...
	protected static function get_log_source() {
		static $ignore_files = array( 'class-sm-log-handler-db', 'class-sm-logger' );
