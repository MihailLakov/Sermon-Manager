 ...
 */
class SM_Log_Handler_File extends SM_Log_Handler {
	 ...
	 *         originally used in deprecated SM_Logger::add calls.
	 * }
	 *
	...
	protected static function format_entry( $timestamp, $level, $message, $context ) {
		...
		if ( isset( $context['_legacy'] ) && true === $context['_legacy'] ) {
			...
			$message = apply_filters( 'sm_logger_add_message', $message, $handle );
	...
	public function remove( $handle ) {
		...
		if ( $file ) {
			...
			do_action( 'sm_log_remove', $handle, $removed );
		}
	...
	public static function get_log_file_path( $handle ) {
		if ( function_exists( 'wp_hash' ) ) {
			return trailingslashit( SM_LOG_DIR ) . sanitize_file_name( $handle . '-' . wp_hash( $handle ) . '.log' );
		} else {
			sm_doing_it_wrong( __METHOD__, __( 'This method should not be called before plugins_loaded.', 'sermon-manager' ), '3.0.0' );
