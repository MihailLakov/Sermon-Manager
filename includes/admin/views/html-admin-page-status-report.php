<?php
/**
 * Admin View: Page - Status Reports
 */
...
<div class="wrap sm">
    <nav class="nav-tab-wrapper sm-nav-tab-wrapper">
		...
		foreach ( $reports as $key => $report_group ) {
			echo '<a href="' . admin_url( 'admin.php?page=sm-reports&tab=' . urlencode( $key ) ) . '" class="nav-tab ';
			if ( $current_tab == $key ) {
				echo 'nav-tab-active';
			}
        ...
        <ul class="subsubsub">
            <li><?php
				...
				foreach ( $reports[ $current_tab ]['reports'] as $key => $report ) {
					...
					$link = '<a href="admin.php?page=sm-reports&tab=' . urlencode( $current_tab ) . '&amp;report=' . urlencode( $key ) . '" class="';
