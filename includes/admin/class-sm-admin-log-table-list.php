...
class SM_Admin_Log_Table_List extends WP_List_Table {
	...
	public function level_dropdown() {
		...
		$levels = array(
			array( 'value' => SM_Log_Levels::EMERGENCY, 'label' => __( 'Emergency', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::ALERT, 'label' => __( 'Alert', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::CRITICAL, 'label' => __( 'Critical', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::ERROR, 'label' => __( 'Error', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::WARNING, 'label' => __( 'Warning', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::NOTICE, 'label' => __( 'Notice', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::INFO, 'label' => __( 'Info', 'sermon-manager' ) ),
			array( 'value' => SM_Log_Levels::DEBUG, 'label' => __( 'Debug', 'sermon-manager' ) ),
		);

		$selected_level = isset( $_REQUEST['level'] ) ? $_REQUEST['level'] : '';
		?>
        <label for="filter-by-level"
               class="screen-reader-text"><?php _e( 'Filter by level', 'sermon-manager' ); ?></label>
        <select name="level" id="filter-by-level">
            <option<?php selected( $selected_level, '' ); ?>
                    value=""><?php _e( 'All levels', 'sermon-manager' ); ?></option>
	...
	public function get_columns() {
		return array(
			...
			'timestamp' => __( 'Timestamp', 'sermon-manager' ),
			'level'     => __( 'Level', 'sermon-manager' ),
			'message'   => __( 'Message', 'sermon-manager' ),
			'source'    => __( 'Source', 'sermon-manager' ),
		);
	}
	...
	public function column_level( $log ) {
		$level_key = SM_Log_Levels::get_severity_level( $log['level'] );
		$levels    = array(
			'emergency' => __( 'Emergency', 'sermon-manager' ),
			'alert'     => __( 'Alert', 'sermon-manager' ),
			'critical'  => __( 'Critical', 'sermon-manager' ),
			'error'     => __( 'Error', 'sermon-manager' ),
			'warning'   => __( 'Warning', 'sermon-manager' ),
			'notice'    => __( 'Notice', 'sermon-manager' ),
			'info'      => __( 'Info', 'sermon-manager' ),
			'debug'     => __( 'Debug', 'sermon-manager' ),
		);
	...
	protected function get_bulk_actions() {
		return array(
			'delete' => __( 'Delete', 'sermon-manager' ),
		);
	}
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' === $which ) {
			...
			submit_button( __( 'Filter', 'sermon-manager' ), '', 'filter-action', false );
			echo '</div>';
		}
	}
	...
	protected function source_dropdown() {
		...
		if ( ! empty( $sources ) ) {
            ...
            <label for="filter-by-source"
                   class="screen-reader-text"><?php _e( 'Filter by source', 'sermon-manager' ); ?></label>
            <select name="source" id="filter-by-source">
                <option<?php selected( $selected_source, '' ); ?>
                        value=""><?php _e( 'All sources', 'sermon-manager' ); ?></option>
	...
	public function prepare_items() {
		...
		$per_page = $this->get_items_per_page( 'sm_status_log_items_per_page', 10 );
		...
		$query_count = "SELECT COUNT(log_id) FROM {$wpdb->prefix}sm_log {$where}";
	...
	protected function get_items_query_where() {
		...
		$where_values     = array();
		if ( ! empty( $_REQUEST['level'] ) && SM_Log_Levels::is_valid_level( $_REQUEST['level'] ) ) {
			$where_conditions[] = 'level >= %d';
			$where_values[]     = SM_Log_Levels::get_level_severity( $_REQUEST['level'] );
		}
		if ( ! empty( $_REQUEST['source'] ) ) {
			$where_conditions[] = 'source = %s';
			$where_values[]     = sm_clean( $_REQUEST['source'] );
		}
