 ...
 */
class SM_Register_WP_Admin_Settings {

	/** @var stdClass Contains the current class to pull settings from. Usually an admin page object */
	protected $object;
	 ...
	 * @since 3.0.0
	 *
	 * @param SM_Settings_Page $object The object that contains the settings to register.
	...
	public function __construct( $object, $type ) {
		...
		if ( 'page' === $type ) {
			add_filter( 'sm_settings_groups', array( $this, 'register_page_group' ) );
			add_filter( 'sm_settings-' . $this->object->get_id(), array( $this, 'register_page_settings' ) );
		}
	}
	...
	public function register_page_settings( $settings ) {
		 ...
		 * a particular setting group (like 'sermons') and register them
		 * to the REST API.
		 */
